...
- [PropertyPanel/stylePlugins/index.ts](file://designer/src/pages/Designer/components/PropertyPanel/stylePlugins/index.ts)
- [PropertyPanel/stylePlugins/types.ts](file://designer/src/pages/Designer/components/PropertyPanel/stylePlugins/types.ts)
- [PropertyPanel/stylePlugins/DefaultStylePlugin.tsx](file://designer/src/pages/Designer/components/PropertyPanel/stylePlugins/DefaultStylePlugin.tsx)
- [PropertyPanel/stylePlugins/TextStylePlugin.tsx](file://designer/src/pages/Designer/components/PropertyPanel/stylePlugins/TextStylePlugin.tsx)
- [PropertyPanel/stylePlugins/TableStylePlugin.tsx](file://designer/src/pages/Designer/components/PropertyPanel/stylePlugins/TableStylePlugin.tsx)
- [PropertyPanel/stylePlugins/LineStylePlugin.tsx](file://designer/src/pages/Designer/components/PropertyPanel/stylePlugins/LineStylePlugin.tsx)
- [PropertyPanel/StyleSection.tsx](file://designer/src/pages/Designer/components/PropertyPanel/StyleSection.tsx)
- [DebugPanel/index.tsx](file://designer/src/pages/Designer/components/DebugPanel/index.tsx)
</cite>

## 更新摘要
**变更内容**
- 属性面板与样式插件架构重构：实现插件化样式配置系统，支持按组件类型动态加载样式编辑器
- 插件注册系统调试增强：添加插件未注册时的警告日志，提供插件注册状态查询功能
- 条件渲染逻辑改进：优化样式区块的显示逻辑，装饰性组件自动隐藏样式配置区域
- 调试面板功能增强：提供模板JSON调试输出和复制功能
...
- **重构的属性面板与样式插件系统**：插件化架构支持按组件类型动态加载样式编辑器
- **调试增强功能**：插件注册系统的调试日志和条件渲染逻辑优化
...
subgraph "属性面板与样式插件系统"
W["PropertyPanel/index.tsx<br/>属性面板主组件"]
X["StyleSection.tsx<br/>样式属性配置区域"]
Y["stylePlugins/<br/>插件注册系统"]
Z["调试增强功能"]
end
A --> B
B --> D
...
- [PropertyPanel/index.tsx:1-152](file://designer/src/pages/Designer/components/PropertyPanel/index.tsx#L1-L152)
- [PropertyPanel/StyleSection.tsx:1-36](file://designer/src/pages/Designer/components/PropertyPanel/StyleSection.tsx#L1-L36)
- [PropertyPanel/stylePlugins/index.ts:1-52](file://designer/src/pages/Designer/components/PropertyPanel/stylePlugins/index.ts#L1-L52)
...
- **重构的属性面板与样式插件系统**
  - **插件化架构**：通过 Map 存储组件类型与样式插件的映射关系
  - **动态样式配置**：根据组件类型动态加载对应的样式编辑器
  - **调试增强**：插件未注册时输出警告日志，提供插件注册状态查询
  - **条件渲染优化**：装饰性组件自动隐藏样式配置区域，提升用户体验
- 统一表单模态框组件
...
## 架构总览
整体采用"路由驱动 + 状态集中 + 统一表单 + 三区域布局 + 插件化样式系统"的架构模式：
...
- **插件化样式系统**：PropertyPanel 采用插件化架构，支持按组件类型动态加载样式编辑器，提供更好的可扩展性。
...
subgraph "插件化样式系统"
PS["PropertyPanel 主组件"]
SS["StyleSection<br/>样式配置区域"]
SP["stylePlugins/<br/>插件注册系统"]
DE["调试增强功能"]
end
P1 --> Z
P1 --> PP
...
- **属性面板主组件**
  ...
  - **新增区域显示功能**：当页头/页脚启用时，显示组件所属区域信息。
- **插件化样式配置系统**
  - **插件注册表**：使用 Map 存储组件类型与样式插件的映射关系
  - **动态样式加载**：根据组件类型动态获取对应的样式插件
  - **调试增强**：插件未注册时输出警告日志，提示使用默认插件
  - **插件注册状态查询**：提供 getRegisteredPlugins 方法获取已注册插件列表
- **样式插件架构**
  - **StylePlugin 接口**：定义插件的标准结构，包括 name 和 render 方法
  - **组件特定插件**：
    - TextStylePlugin：文本组件的样式配置（字体大小、字重、对齐方式、颜色）
    - TableStylePlugin：表格组件的样式配置（字体大小、对齐方式）
    - LineStylePlugin：线条组件的样式配置（高度、样式、颜色）
    - DefaultStylePlugin：默认插件，装饰性组件无需样式配置
  - **条件渲染优化**：装饰性组件（image、qrcode、barcode、rect）自动隐藏样式配置区域
- **调试增强功能**
  - 插件注册系统的调试日志输出
  - 插件未找到时的警告提示
  - 插件注册状态的查询接口
...
- [PropertyPanel/StyleSection.tsx:1-36](file://designer/src/pages/Designer/components/PropertyPanel/StyleSection.tsx#L1-L36)
- [PropertyPanel/stylePlugins/index.ts:1-52](file://designer/src/pages/Designer/components/PropertyPanel/stylePlugins/index.ts#L1-L52)
- [PropertyPanel/stylePlugins/types.ts:1-31](file://designer/src/pages/Designer/components/PropertyPanel/stylePlugins/types.ts#L1-L31)
...
- [PropertyPanel/stylePlugins/index.ts:1-52](file://designer/src/pages/Designer/components/PropertyPanel/stylePlugins/index.ts#L1-L52)
- [PropertyPanel/stylePlugins/types.ts:1-31](file://designer/src/pages/Designer/components/PropertyPanel/stylePlugins/types.ts#L1-L31)
- [PropertyPanel/stylePlugins/DefaultStylePlugin.tsx:1-16](file://designer/src/pages/Designer/components/PropertyPanel/stylePlugins/DefaultStylePlugin.tsx#L1-L16)
- [PropertyPanel/stylePlugins/TextStylePlugin.tsx:1-86](file://designer/src/pages/Designer/components/PropertyPanel/stylePlugins/TextStylePlugin.tsx#L1-L86)
- [PropertyPanel/stylePlugins/TableStylePlugin.tsx:1-48](file://designer/src/pages/Designer/components/PropertyPanel/stylePlugins/TableStylePlugin.tsx#L1-L48)
- [PropertyPanel/stylePlugins/LineStylePlugin.tsx:1-72](file://designer/src/pages/Designer/components/PropertyPanel/stylePlugins/LineStylePlugin.tsx#L1-L72)

### 调试面板功能增强
- **模板JSON调试输出**
  - 实时生成并显示当前设计器状态的模板JSON
  - 提供一键复制功能，方便开发者调试和分享
  - 显示模板基本信息：名称、Schema ID、组件数量统计
- **组件统计信息**
  - 显示页头、内容、页脚区域的组件数量分布
  - 统计总组件数量，便于性能分析和问题定位
- **调试面板交互**
  - 固定在右下角的悬浮按钮，支持展开/收起
  - Drawer 组件提供完整的调试界面
  - 支持键盘快捷键（Ctrl++/Ctrl+-）进行缩放操作

```mermaid
flowchart TD
Open["点击调试按钮"] --> Drawer["展开调试抽屉"]
Drawer --> Info["显示模板信息"]
Info --> Stats["显示组件统计"]
Stats --> JSON["显示模板JSON"]
JSON --> Copy["复制JSON按钮"]
...
**图表来源**
- [DebugPanel/index.tsx:1-77](file://designer/src/pages/Designer/components/DebugPanel/index.tsx#L1-L77)
- [Designer.tsx:19:246](file://designer/src/pages/Designer/index.tsx#L19-L246)
...
- **三区域布局优化**
  ...
  - 区域高度调整的性能优化
- **插件化样式系统优化**
  - 插件注册表使用 Map 提供 O(1) 查找性能
  - 插件缓存机制避免重复创建插件实例
  - 条件渲染优化，装饰性组件不渲染样式配置区域
...
- **区域感知操作问题**
  ...
  - 验证表格组件的区域限制逻辑
- **属性面板与样式插件问题**
  - 检查插件注册系统是否正常工作，确认插件是否正确注册
  - 确认 getPlugin 函数能够正确返回对应插件
  - 验证样式配置区域的条件渲染逻辑
  - 检查调试日志输出，确认插件未注册时的警告信息
- **调试面板问题**
  - 检查调试面板的展开/收起功能是否正常
  - 确认模板JSON生成和复制功能
  - 验证组件统计信息的准确性
...
**本次重大更新显著增强了三区域布局支持和属性面板系统**：
- **完整的三区域状态管理**：页头、内容、页脚三大区域的独立状态管理与协同工作
- **区域感知的组件操作**：对齐、分布、层级管理均支持跨区域操作，提供更灵活的设计体验
- **增强的拖拽状态管理**：实时拖拽状态跟踪、跨区域拖拽判断、拖拽手柄状态管理
- **区域移动功能**：支持组件在三大区域间的移动与跨区域对齐，表格组件的区域限制逻辑
- **重构的属性面板与样式插件系统**：实现插件化架构，支持按组件类型动态加载样式编辑器
- **插件注册系统的调试增强**：添加插件未注册时的警告日志，提供插件注册状态查询功能
- **条件渲染逻辑改进**：装饰性组件自动隐藏样式配置区域，提升用户体验
- **调试面板功能增强**：提供模板JSON调试输出和复制功能
...
**插件化样式系统**提供了更好的可扩展性和维护性：
- 通过 Map 存储组件类型与样式插件的映射关系，提供 O(1) 查找性能
- 动态样式配置支持，按组件类型加载对应的样式编辑器
- 调试增强功能，插件未注册时输出警告日志
- 条件渲染优化，装饰性组件自动隐藏样式配置区域
