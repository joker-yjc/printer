...
## 更新摘要
**变更内容**
- 新增中文大写数字管道配置器（ChineseNumberPipeConfigurator）
- MoneyPipe配置器功能增强，支持更多输出格式和转换选项
- ChineseNumberPipe执行器集成到管道系统中
- MoneyPipe执行器新增中文大写金额转换功能
...
**更新** 新增中文大写数字管道支持，增强金额转换功能，提供更丰富的数据格式化选项。

## 项目结构
数据绑定系统由三层组成：
- 设计器前端（React + Zustand 状态管理）：负责可视化编辑、属性面板配置、Mock 数据生成与 API 交互。
- SDK 管道引擎（TypeScript）：提供管道执行器注册、执行与类型定义，支持日期、货币、金额、中文大写数字等格式化。
...
ChineseCfg["中文大写配置器<br/>ChineseNumberPipeConfigurator"]
MoneyCfg["金额转换配置器<br/>MoneyPipeConfigurator"]
end
subgraph "SDK 管道引擎"
...
ChineseNumPipe["ChineseNumberPipe 执行器"]
end
subgraph "服务器后端"
...
**更新** 新增中文大写数字管道配置器，支持数字到中文大写的转换，以及金额转换管道的增强功能。
...
participant E as "执行器<br/>DatePipe/CurrencyPipe/MoneyPipe/ChineseNumberPipe"
U->>P : 设置绑定路径/回退值/添加管道
P->>C : 获取配置器(type)
C-->>P : 返回对应配置器
...
- [designer/src/pipes/configurators/index.tsx:68-84](file://designer/src/pipes/configurators/index.tsx#L68-L84)
...
**图示来源**
- [designer/src/pages/Designer/components/PropertyPanel/DataBindingSection.tsx:10-11](file://designer/src/pages/Designer/components/PropertyPanel/DataBindingSection.tsx#L10-L11)
- [designer/src/pipes/configurators/index.tsx:80-84](file://designer/src/pipes/configurators/index.tsx#L80-L84)
- [sdk/src/pipes/registry.ts:31-40](file://sdk/src/pipes/registry.ts#L31-L40)
...
- 内置配置器
  ...
  - MoneyPipeConfigurator：金额转换，支持分/元互转、精度、千分位与符号，**更新**支持中文大写输出格式。
  - ChineseNumberPipeConfigurator：**新增**中文大写数字转换，支持输出模式与连接符配置。
  - SlicePipeConfigurator：字符串截取，起止位置。
  - DefaultPipeConfigurator：默认值。
- 注册与获取
  - configuratorRegistry：Map 存储，getConfigurator(type) 获取配置器。

**更新** 新增 ChineseNumberPipeConfigurator 配置器，MoneyPipeConfigurator 增强支持中文大写金额输出。
...
**图示来源**
- [designer/src/pipes/configurators/index.tsx:16-20](file://designer/src/pipes/configurators/index.tsx#L16-L20)
...
- [designer/src/pipes/configurators/MoneyPipeConfigurator.tsx:9-143](file://designer/src/pipes/configurators/MoneyPipeConfigurator.tsx#L9-L143)
...
- 内置执行器
  ...
  - MoneyPipe：金额转换（分/元），支持精度、千分位与符号，**更新**支持中文大写金额输出，使用高精度库避免浮点误差。
  - ChineseNumberPipe：**新增**中文大写数字转换，支持数字到中文大写的转换，支持输出模式与连接符配置。

**更新** ChineseNumberPipe 执行器新增，MoneyPipe 执行器增强支持中文大写金额转换。
...
- 数据绑定示例
  ...
  - **新增** 使用 chineseNumber 管道将数字转换为中文大写形式，支持输出模式与连接符配置。
...
**更新** 新增 ChineseNumberPipeConfigurator 与 ChineseNumberPipe 执行器的依赖关系。
...
Reg --> Types["类型定义"]
Reg --> Execs["执行器集合"]
Reg --> ChineseNumPipe["ChineseNumberPipe 执行器"]
...
- 管道执行
  ...
  - **新增** ChineseNumberPipe 使用高效的中文大写转换算法，支持大数字范围转换。
- Mock 数据生成
...
- **新增** 中文大写转换错误
  - 现象：中文大写转换失败或输出异常。
  - 排查：确认输入为合法数值，检查中文大写转换算法是否支持该数字范围。
- Mock 数据为空
  - 现象：绑定路径无数据显示。
...
## 结论
该数据绑定系统以 Schema 为核心，结合可视化属性面板与可扩展的管道系统，实现了从数据建模到实时渲染的完整闭环。通过前后端分离的 API 设计与内置示例数据，开发者可以快速搭建与调试打印模板。**更新** 新增的中文大写数字管道与增强的金额转换功能，进一步丰富了数据格式化能力，满足更多业务场景需求。建议在生产环境中加强输入校验、错误日志与性能监控，确保复杂场景下的稳定性与可维护性。
...
- 最佳实践
  ...
  - **新增** 在财务相关场景使用 MoneyPipe 的中文大写输出格式，提高票据的专业性。
  - **新增** 使用 ChineseNumberPipe 进行金额大写转换时，注意连接符的配置以满足不同业务需求。
- 快速上手步骤
  - 在 Schema 管理中选择内置示例，复制其 ID。
  - 在 Mock 数据中选择匹配的示例数据，或创建新数据。
  - 在属性面板中设置 DataBinding 的 path 与管道，实时预览效果。
  - 导出模板时，确保 schemaId 与 page 配置正确。
  - **新增** 在金额字段中使用 MoneyPipe 的中文大写格式，或使用 ChineseNumberPipe 进行通用数字大写转换。