...
## 更新摘要
**变更内容**
- 新增坐标转换逻辑和 contentTop/contentBottom 坐标系统支持
- 分页精度改进，采用设计时相对间距模型
- 页头/页脚区域集成，支持 headerComponents/footerComponents
- 页面区域划分机制，组件按 header/content/footer 自动定位
- 增强的分页计算算法，支持表格跨页精确控制
- 改进的 HTML 模板生成，支持页头/页脚样式处理
...
- **坐标转换与相对间距模型**：基于设计时坐标系统的精确分页计算
- **页头/页脚区域集成**：支持 headerComponents/footerComponents 的智能布局
- **contentTop/contentBottom 坐标系统**：页面内容区域的精确坐标管理
...
**图表来源**
- [sdk/src/printEngine.ts:31-1095](file://sdk/src/printEngine.ts#L31-L1095)
...
- 插件化渲染器：TextRenderer、ImageRenderer、RectRenderer、LineRenderer、TableRenderer、QRCodeRenderer、BarcodeRenderer、PageNumberRenderer
...
-calculatePages(components, headerComponents, footerComponents) ComponentNode[][]
...
- 连续纸模式：不分页，单页渲染
- **页头/页脚集成**：支持 headerComponents/footerComponents 的智能布局和坐标转换

```mermaid
flowchart TD
Start(["开始"]) --> CheckSize["检测页面尺寸<br/>A4/A5/CUSTOM/CONTINUOUS"]
CheckSize --> Avail["计算可用高度<br/>heightMm - marginTop - marginBottom - headerHeight - footerHeight"]
Avail --> ContentTop["计算 contentTop<br/>marginTop + headerHeight"]
ContentTop --> Sort["按 yMm 升序排序组件"]
...
- 默认渲染器：TextRenderer、ImageRenderer、RectRenderer、LineRenderer、TableRenderer、QRCodeRenderer、BarcodeRenderer、PageNumberRenderer
...
**新增** 打印引擎引入了全新的坐标转换逻辑和页面区域系统，支持 contentTop/contentBottom 坐标系统

#### 坐标转换机制
- **设计时坐标系**：组件使用设计时的绝对坐标（xMm, yMm）
- **页面坐标转换**：将设计坐标转换为页面坐标，考虑页头/页脚高度
- **contentTop 计算**：`contentTop = marginTop + headerHeight`
- **contentBottom 计算**：`contentBottom = heightMm - marginBottom - footerHeight`

#### 页面区域划分
- **header 区域**：位于页面顶部，高度由 headerHeight 决定
- **content 区域**：位于页头和页脚之间，是主要的打印内容区域
- **footer 区域**：位于页面底部，高度由 footerHeight 决定

#### 坐标系统支持
- **相对间距模型**：保留设计时的相对间距，支持组件重叠
- **页头/页脚坐标转换**：页头组件 yMm 相对于 marginTop，页脚组件 yMm 相对于 contentBottom
- **表格跨页处理**：表格组件的坐标转换考虑页头/页脚高度

```mermaid
flowchart TD
DesignCoord["设计时坐标<br/>(xMm, yMm)"] --> CheckSection{"检查组件区域"}
CheckSection --> |header| HeaderTransform["页头转换<br/>yMm += marginTop"]
CheckSection --> |footer| FooterTransform["页脚转换<br/>yMm = heightMm - marginBottom - componentHeight - yMm"]
CheckSection --> |content| ContentTransform["内容转换<br/>yMm += headerHeight"]
HeaderTransform --> PageCoord["页面坐标"]
FooterTransform --> PageCoord
ContentTransform --> PageCoord
PageCoord --> FinalRender["最终渲染"]
```

**图表来源**
- [sdk/src/printEngine.ts:434-445](file://sdk/src/printEngine.ts#L434-L445)
- [sdk/src/printEngine.ts:592-614](file://sdk/src/printEngine.ts#L592-L614)
- [designer/src/store/designer.ts:592-608](file://designer/src/store/designer.ts#L592-L608)
- [designer/src/pages/Designer/components/Canvas/index.tsx:659-674](file://designer/src/pages/Designer/components/Canvas/index.tsx#L659-L674)

#### 核心实现细节
- **页头有效高度**：`headerHeight = max(page.headerHeight, measureMaxBottom(headerComponents))`
- **页脚有效高度**：`footerHeight = max(page.footerHeight, measureMaxBottom(footerComponents))`
- **contentTop 计算**：`contentTop = marginTop + headerHeight`
- **availableHeightMm 计算**：`availableHeightMm = heightMm - marginTop - marginBottom - headerHeight - footerHeight`

**章节来源**
- [sdk/src/printEngine.ts:434-445](file://sdk/src/printEngine.ts#L434-L445)
- [sdk/src/printEngine.ts:592-614](file://sdk/src/printEngine.ts#L592-L614)
- [sdk/src/printEngine.ts:406-409](file://sdk/src/printEngine.ts#L406-L409)
- [designer/src/store/designer.ts:592-608](file://designer/src/store/designer.ts#L592-L608)
- [designer/src/pages/Designer/components/Canvas/index.tsx:659-674](file://designer/src/pages/Designer/components/Canvas/index.tsx#L659-L674)

### 页头/页脚区域集成（核心增强）

**新增** 打印引擎现在支持完整的页头/页脚区域系统，实现智能布局和坐标转换

#### 区域配置
- **headerEnabled**：控制页头区域是否启用
- **footerEnabled**：控制页脚区域是否启用
- **headerHeight**：页头区域高度（mm），默认自动计算
- **footerHeight**：页脚区域高度（mm），默认自动计算

#### 智能布局机制
- **页头组件处理**：将 headerComponents 的 yMm 相对于 marginTop 进行转换
- **页脚组件处理**：将 footerComponents 的 yMm 相对于 contentBottom 进行转换
- **内容组件处理**：将 content 区域组件的 yMm 相对于 contentTop 进行转换

#### 坐标转换算法
- **页头组件**：`yMm = (h.layout.yMm || 0) + marginTop`
- **页脚组件**：`yMm = (f.layout.yMm || 0) + heightMm - footerHeight - marginBottom`
- **内容组件**：`yMm = (c.layout.yMm || 0) + headerHeight`

#### 分页精度改进
- **测量最大底部**：使用 `measureMaxBottom()` 计算页头/页脚的实际占用高度
- **动态高度计算**：根据实际渲染高度动态调整可用内容区域
- **精确边界控制**：确保页头/页脚与内容区域的精确分隔

**章节来源**
- [sdk/src/printEngine.ts:434-445](file://sdk/src/printEngine.ts#L434-L445)
- [sdk/src/printEngine.ts:592-614](file://sdk/src/printEngine.ts#L592-L614)
- [sdk/src/printEngine.ts:406-409](file://sdk/src/printEngine.ts#L406-L409)
- [sdk/src/types.ts:62-69](file://sdk/src/types.ts#L62-L69)
...
- **坐标转换异常**：
  - 页头/页脚高度计算错误：检查 headerHeight/footerHeight 配置
  - contentTop 计算错误：验证 marginTop 和 headerHeight 设置
  - 组件坐标转换异常：检查组件的 _section 属性设置
- **分页精度问题**：
  - 表格跨页边界错误：检查表格高度和可用高度计算
  - 页头/页脚遮挡内容：调整 headerHeight/footerHeight 设置
  - 组件重叠问题：检查设计时相对间距设置
...
**关键更新增强了引擎的实用性和可靠性**：
- **坐标转换与相对间距模型**：基于设计时坐标系统的精确分页计算，支持组件重叠和精确边界控制
- **页头/页脚区域集成**：完整的页头/页脚区域系统，支持智能布局和坐标转换
- **contentTop/contentBottom 坐标系统**：页面内容区域的精确坐标管理，确保页眉页脚与内容的精确分隔
...
- **配置页头/页脚**：设置 headerEnabled/headerHeight/footerEnabled/footerHeight 参数
