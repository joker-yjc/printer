...
- [designer/node_modules/@tomjs/vite-plugin-monaco-editor/package.json](file://designer/node_modules/@tomjs/vite-plugin-monaco-editor/package.json)
- [debug-print-offset.html](file://debug-print-offset.html)
- [README.md](file://README.md)
</cite>

## 更新摘要
**变更内容**
- 修复打印内容偏移问题，改进 @media print 规则，确保边距完全覆盖
- 优化 .print-page 样式，移除 padding 以实现组件坐标直接映射到纸张坐标系
- 增强 @media print 样式覆盖机制，解决打印内容偏左问题
- 改进批量打印样式，确保边距功能正常生效
- 新增打印偏移问题调试工具，提供可视化对比分析
...
## 项目结构
打印引擎位于 SDK 子模块中，采用"插件化渲染器 + 统一模板生成"的架构组织：
...
## 架构总览
PrintEngine 以"插件化渲染器 + 统一模板生成"为核心，形成清晰的职责分离：
...
- [sdk/src/printEngine.ts:563-685](file://sdk/src/printEngine.ts#L563-L685)
- [sdk/src/printEngine.ts:691-837](file://sdk/src/printEngine.ts#L691-L837)
...
- [sdk/src/printEngine/htmlTemplate.ts:248-273](file://sdk/src/printEngine/htmlTemplate.ts#L248-L273)
...
- [sdk/src/printEngine.ts:563-685](file://sdk/src/printEngine.ts#L563-L685)
- [sdk/src/printEngine.ts:691-837](file://sdk/src/printEngine.ts#L691-L837)
- [sdk/src/printEngine/renderers/TableRenderer.ts:14-176](file://sdk/src/printEngine/renderers/TableRenderer.ts#L14-L176)
...
**章节来源**
- [designer/package.json:31](file://designer/package.json#L31)
- [designer/vite.config.ts:11](file://designer/vite.config.ts#L11)
- [designer/node_modules/@tomjs/vite-plugin-monaco-editor/package.json:1-88](file://designer/node_modules/@tomjs/vite-plugin-monaco-editor/package.json#L1-L88)
...
**章节来源**
- [sdk/src/printEngine.ts:80-105](file://sdk/src/printEngine.ts#L80-L105)
- [sdk/src/printEngine.ts:110-126](file://sdk/src/printEngine.ts#L110-L126)
- [sdk/src/printEngine.ts:131-151](file://sdk/src/printEngine.ts#L131-L151)
- [sdk/src/pipes/registry.ts:45-64](file://sdk/src/pipes/registry.ts#L45-L64)
...
**图表来源**
- [debug-print-offset.html:371-406](file://debug-print-offset.html#L371-L406)
...
**新增** 提供可视化调试工具，帮助开发者理解和验证打印偏移修复效果

#### 工具功能
- **边距可视化**：实时显示上、右、下、左四个边距区域
- **组件位置对比**：绿色预期位置 vs 红色实际位置（修复前）
- **偏移指示**：显示具体的偏移量（mm）
- **交互式调整**：拖动滑块实时调整边距参数

#### 调试场景
- **设计视图**：画布无 padding，组件坐标直接映射到纸张边缘
- **修复前视图**：`.print-page` 有 padding，组件整体右移
- **修复后视图**：`.print-page` 无 padding，与设计视图一致
- **差异叠加视图**：同时显示预期和实际位置，直观对比偏移

**章节来源**
- [debug-print-offset.html:1-610](file://debug-print-offset.html#L1-L610)
...
## 结论
打印引擎通过"插件化渲染器 + 统一模板生成"的架构，实现了可扩展、可维护的打印能力。**革命性的测量后渲染方法彻底解决了表格分页中的长文本内容中断问题，通过实时 DOM 测量实现了精确的分页边界控制**。相对间距分页模型兼顾易用性与可控性，HTML 模板生成器保证了样式与结构的一致性。

**关键更新增强了引擎的实用性和可靠性**：
- **打印内容偏移修复**：通过改进 @media print 规则和移除 padding，确保边距完全覆盖，解决打印内容偏移问题
- 嵌套对象数据结构支持使得表格能够处理复杂的业务数据
- Monaco Editor 性能优化提升了开发者的编辑体验
- 新增的调试工具帮助开发者更好地理解和验证修复效果
...
- [sdk/src/printEngine.ts:563-685](file://sdk/src/printEngine.ts#L563-L685)
- [sdk/src/printEngine.ts:691-837](file://sdk/src/printEngine.ts#L691-L837)
- [docs/需求文档.md:231-261](file://docs/需求文档.md#L231-L261)
