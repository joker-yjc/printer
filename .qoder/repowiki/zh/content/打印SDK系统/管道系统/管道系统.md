...
## 简介
本文件系统性地介绍"管道系统"的设计与实现，涵盖注册器模式与执行器模式的应用；详解内置执行器（DatePipe、CurrencyPipe、MoneyPipe、ChineseNumberPipe 等）的设计原理与行为；阐述管道注册机制（执行器注册流程、参数校验与错误处理）；介绍管道配置系统（管道链组合、参数传递与执行顺序控制）；并提供自定义管道开发指南（接口实现、注册方法与测试策略），以及性能优化与最佳实践建议。
...
PExecCN["sdk/src/pipes/executors/ChineseNumberPipe.ts"]
end
...
- [sdk/src/pipes/executors/index.ts:6-45](file://sdk/src/pipes/executors/index.ts#L6-L45)
- [designer/src/pipes/configurators/index.tsx:16-85](file://designer/src/pipes/configurators/index.tsx#L16-L85)
- [sdk/src/types.ts:66-75](file://sdk/src/types.ts#L66-L75)
...
- [designer/src/pipes/configurators/index.tsx:82-85](file://designer/src/pipes/configurators/index.tsx#L82-L85)
...
### 内置执行器：MoneyPipe
- 功能：金额转换与格式化，支持分转元、元转分、千分位分隔与货币符号，**新增中文大写金额转换**。
- 参数：mode（默认分转元）、precision（默认精度）、symbol（货币符号）、separator（千分位开关）、format（输出格式：number/chineseUppercase）。
- 处理流程：使用高精度库进行计算；根据模式切换；**当format为chineseUppercase时，支持中文大写金额转换**；格式化后按需加千分位与货币符号；异常时回退为原始值。
...
Mode --> |none| None["不转换"]
FenToYuan --> FormatCheck{"格式化类型"}
YuanToFen --> FormatCheck
None --> FormatCheck
FormatCheck --> |chineseUppercase| ChineseMoney["中文大写金额转换"]
FormatCheck --> |number| FormatNum["按精度格式化"]
ChineseMoney --> Separator{"启用千分位？"}
FormatNum --> Separator
...
**图表来源**
- [sdk/src/pipes/executors/MoneyPipe.ts:13-129](file://sdk/src/pipes/executors/MoneyPipe.ts#L13-L129)

**章节来源**
- [sdk/src/pipes/executors/MoneyPipe.ts:1-130](file://sdk/src/pipes/executors/MoneyPipe.ts#L1-L130)

### 内置执行器：ChineseNumberPipe
- 功能：**新增**中文大写数字转换，支持整数、小数、负数处理，可选择输出模式与自定义连接符。
- 参数：mode（输出模式：uppercase/both）、separator（连接符，仅在both模式下使用）、unit（后缀单位）。
- 处理流程：支持负数前缀"负"；小数部分逐位读出"点"；可选择仅输出大写或同时显示原值；支持自定义连接符或使用默认括号。

```mermaid
flowchart TD
Start(["开始"]) --> NullCheck{"值为空/未定义/空串？"}
NullCheck --> |是| ReturnEmpty["返回空字符串"]
NullCheck --> |否| ParseNum["转换为数值"]
ParseNum --> Valid{"数值有效？"}
Valid --> |否| Fallback["回退原始值或空字符串"]
Valid --> |是| NegCheck{"是否为负数？"}
NegCheck --> |是| AddNeg["添加'负'前缀"] --> Convert["转换为正数大写"]
NegCheck --> |否| Convert["转换为大写"]
Convert --> DotCheck{"是否有小数部分？"}
DotCheck --> |是| IntPart["转换整数部分"]
IntPart --> AddDot["添加'点'"]
AddDot --> DecPart["逐位转换小数部分"]
DecPart --> UnitCheck{"是否有单位？"}
UnitCheck --> |是| AddUnit["添加单位"]
UnitCheck --> |否| NoUnit["保持原样"]
AddUnit --> ModeCheck{"输出模式"}
NoUnit --> ModeCheck
ModeCheck --> |both| AddConnector["添加连接符"]
ModeCheck --> |uppercase| Final["返回最终结果"]
AddConnector --> Final
```

**图表来源**
- [sdk/src/pipes/executors/ChineseNumberPipe.ts:17-137](file://sdk/src/pipes/executors/ChineseNumberPipe.ts#L17-L137)
...
- UI 配置器：根据管道类型渲染对应参数控件（如日期格式、货币符号、金额模式、精度、千分位、**中文大写模式与连接符**等），并将变更写回 PipeConfig。
...
- 运行时依赖：
  - MoneyPipe 依赖高精度计算库；DatePipe 依赖浏览器原生 Date；CurrencyPipe 依赖数值格式化；**ChineseNumberPipe 依赖 toChineseUppercaseInteger 函数**。
- 循环依赖：
...
- **中文大写转换性能**：ChineseNumberPipe 的中文大写转换涉及字符串拼接与循环处理，对于大量数据处理时应考虑性能影响。
...
- 货币/金额格式化异常：
  - 现象：异常时回退为原始值。
  - 排查：确认数值可转换为高精度数；检查 mode 与 precision 合法范围；确认千分位与货币符号设置；**检查中文大写格式参数**。
- **中文大写转换异常**：
  - 现象：ChineseNumberPipe 转换失败或输出异常。
  - 排查：确认数值范围（支持到亿级）；检查小数部分处理；验证负数处理逻辑；检查连接符设置。
- UI 配置器不生效：
  - 现象：添加管道后无法渲染配置控件。
...
- [sdk/src/pipes/registry.ts:45-52](file://sdk/src/pipes/registry.ts#L45-L52)
- [sdk/src/pipes/executors/DatePipe.ts:11-34](file://sdk/src/pipes/executors/DatePipe.ts#L11-L34)
- [sdk/src/pipes/executors/MoneyPipe.ts:124-129](file://sdk/src/pipes/executors/MoneyPipe.ts#L124-L129)
- [sdk/src/pipes/executors/ChineseNumberPipe.ts:132-137](file://sdk/src/pipes/executors/ChineseNumberPipe.ts#L132-L137)
- [designer/src/pipes/configurators/index.tsx:82-85](file://designer/src/pipes/configurators/index.tsx#L82-L85)

## 结论
该管道系统通过注册器模式与执行器模式实现了清晰的职责分离与良好的扩展性；UI 配置器与执行器解耦，既保证了易用性又便于维护；内置执行器覆盖常用格式化需求，并提供高精度金额处理能力。**新增的ChineseNumberPipe提供了完整的中文大写数字转换功能，支持小数、负数处理与多种输出模式；MoneyPipe的增强使其能够生成符合会计规范的中文大写金额**。结合渲染上下文的管道链应用，系统能够稳定地完成数据格式化与展示准备。
...
#### ChineseNumberPipe 中文大写数字转换
- **功能特性**：
  - 支持整数、小数、负数的完整中文大写转换
  - 提供两种输出模式：仅大写（如"壹仟"）和原值+大写（如"1000（壹仟）"）
  - 支持自定义连接符，在both模式下可指定任意连接符
  - 支持后缀单位添加，如"元"、"万元"等
- **技术实现**：
  - 使用中文数字字符映射表：零、壹、贰、叁、肆、伍、陆、柒、捌、玖
  - 支持十进制单位：拾、佰、仟
  - 支持大单位：万、亿
  - 小数部分逐位读出，使用"点"连接
  - 负数自动添加"负"前缀

#### MoneyPipe 增强中文大写金额支持
- **功能增强**：
  - 新增中文大写金额格式化选项
  - 支持会计规范的元、角、分处理
  - 支持"整"字的正确添加
  - 与ChineseNumberPipe共享中文大写转换逻辑
- **技术实现**：
  - 集成toChineseUppercaseInteger函数
  - 按照会计规范处理角分转换
  - 支持原值与中文大写的组合输出
  - 保持原有的分转元、元转分、千分位等功能
