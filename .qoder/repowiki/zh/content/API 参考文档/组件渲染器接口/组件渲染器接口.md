...
**更新** 本版本新增了表格样式系统和列级样式支持的详细说明，包括列级样式映射、合并机制和设计器中的样式配置界面。
...
Utils["styleBuilder.ts<br/>样式构建工具"]
TR["TextRenderer.ts"]
IR["ImageRenderer.ts"]
TBLR["TableRenderer.ts<br/>新增列级样式支持"]
RR["RectRenderer.ts"]
LR["LineRenderer.ts"]
...
TP["TextPreview.tsx"]
TTP["TablePreview.tsx<br/>新增列宽拖拽与样式配置"]
TSP["TableStylePlugin.tsx<br/>表格样式插件"]
TCS["TableColumnSection.tsx<br/>列级样式配置界面"]
end
RIdx --> TR
RIdx --> IR
RIdx --> TBLR
...
- [TableStylePlugin.tsx](file://designer/src/pages/Designer/components/PropertyPanel/stylePlugins/TableStylePlugin.tsx)
- [TableColumnSection.tsx:854-898](file://designer/src/pages/Designer/components/PropertyPanel/TableColumnSection.tsx#L854-L898)
...
**更新** 新增表格列级样式支持：
- 列级样式映射：COL_STYLE_MAP 定义了列级样式字段与 CSS 属性的映射关系
- 样式合并：mergeColumnStyle 函数将列级样式合并到基础样式对象中
- 支持的列级样式：fontSize、fontWeight、color、backgroundColor、textAlign
...
class ImageRenderer
class TableRenderer {
+mergeColumnStyle(colStyle, base) object
+COL_STYLE_MAP constant
}
class RectRenderer
class LineRenderer
...
- 渲染逻辑要点
  ...
  - **新增** 列级样式支持：通过 mergeColumnStyle 函数将列级样式合并到单元格样式中，支持字体大小、字重、颜色、背景色和文本对齐等样式属性。

**更新** 新增列级样式功能详解：

列级样式映射机制：
- COL_STYLE_MAP 定义了列级样式字段与 CSS 属性的映射关系
- mergeColumnStyle 函数负责将列级样式合并到基础样式对象中
- 支持的样式属性：fontSize（px 单位）、fontWeight、color、backgroundColor、textAlign

列级样式应用流程：
1. 从列配置中提取 col.style 对象
2. 调用 mergeColumnStyle(col.style, baseStyle) 进行样式合并
3. 将合并后的样式应用到单元格元素
4. 支持空值检测和默认值回退
...
- 关键实现路径
  ...
  - **新增** [TableRenderer.mergeColumnStyle:57-73](file://sdk/src/printEngine/renderers/TableRenderer.ts#L57-L73)
...
Body --> ColumnStyle["应用列级样式<br/>mergeColumnStyle"]
ColumnStyle --> Summary["渲染合计行<br/>renderSummary"]
...
- [TableRenderer.ts:57-73](file://sdk/src/printEngine/renderers/TableRenderer.ts#L57-L73)
...
- 表格预览 TablePreview
  ...
  - **更新** 新增列级样式配置支持：通过 TableStylePlugin 和 TableColumnSection 提供列级样式的可视化配置界面
  - 关键实现路径
...
Types --> TBLR["TableRenderer<br/>新增列级样式"]
...
- 渲染性能
  ...
  - **新增** 列级样式合并：mergeColumnStyle 函数采用浅拷贝和条件判断，避免不必要的样式处理。
- 缓存机制
...
- **新增** 列级样式不生效
  - 现象：表格列的样式配置未在渲染结果中体现。
  - 处理：检查列级样式对象是否正确传递到 mergeColumnStyle 函数，确认样式字段名与 COL_STYLE_MAP 映射一致。
  - 参考路径
    - [TableRenderer.mergeColumnStyle:57-73](file://sdk/src/printEngine/renderers/TableRenderer.ts#L57-L73)
    - [TableRenderer.COL_STYLE_MAP:42-49](file://sdk/src/printEngine/renderers/TableRenderer.ts#L42-L49)
...
- [TablePreview.tsx:69-174](file://designer/src/pages/Designer/components/Canvas/componentRenderers/TablePreview.tsx#L69-L174)
- [TableRenderer.ts:57-73](file://sdk/src/printEngine/renderers/TableRenderer.ts#L57-L73)
- [TableRenderer.ts:42-49](file://sdk/src/printEngine/renderers/TableRenderer.ts#L42-L49)

## 结论
组件渲染器系统以统一接口为核心，配合样式工具与默认常量，实现了高内聚、低耦合的渲染能力。内置渲染器覆盖文本、图片、表格、几何图形、二维识别码与页码等常用组件，满足打印场景需求。通过 RenderContext 提供的数据与上下文能力，渲染器可灵活接入业务数据与管道转换。设计器预览渲染器确保设计期与运行期的一致性体验。

**更新** 新版本增强了表格渲染器的功能，新增了完整的列级样式支持系统，包括：
- 列级样式映射机制
- 动态样式合并功能
- 设计器端的可视化配置界面
- 完整的样式继承和覆盖规则

建议在扩展新渲染器时遵循现有接口与工具规范，并关注性能与安全。
...
#### 列级样式映射机制
- COL_STYLE_MAP 定义了列级样式字段与 CSS 属性的映射关系
- 支持的样式属性：fontSize、fontWeight、color、backgroundColor、textAlign
- 特殊处理：fontSize 自动转换为 px 单位

#### 样式合并策略
- mergeColumnStyle 函数负责将列级样式合并到基础样式对象中
- 空值检测：跳过 undefined、null 和空字符串的样式值
- 优先级：列级样式优先于表格级样式
- 默认值回退：当列级样式缺失时，使用表格默认样式

#### 设计器样式配置
- TableStylePlugin：提供表格级别的样式配置界面
- TableColumnSection：提供列级样式的可视化配置
- 支持的配置项：字体大小、颜色、对齐方式、背景色等
- 实时预览：配置更改即时反映在预览界面中
