...
- [designer/.gitignore](file://designer/.gitignore)
- [designer/src/help/chapters.ts](file://designer/src/help/chapters.ts)
</cite>

## 更新摘要
**所做更改**
- 新增文档管理系统迁移章节，说明从集成Git wiki到外部Qoder系统的变更
- 更新文档管理相关配置和排除规则说明
- 强调文档系统与技术栈的集成关系
...
- 文档管理系统：从集成Git wiki迁移到外部Qoder系统，提升文档协作效率
...
subgraph "文档系统"
QODER["外部Qoder系统"]
CHAPTERS["designer/src/help/chapters.ts"]
end
D_PKG --> D_VITE
D_APP --> D_STORE
...
- 文档管理系统
  - 外部Qoder系统：替代集成Git wiki，提供更好的文档协作与管理功能
  - .gitignore 排除规则：确保文档系统生成的临时文件不被版本控制
...
## 架构总览
整体架构由"设计器前端 + 打印 SDK + 文档系统"组成，三者通过模板数据进行解耦协作：
...
### 文档系统迁移概述
项目文档管理系统已从集成Git wiki迁移到外部Qoder系统，这一变更提升了文档协作效率与管理便利性：
...
### 文档配置与排除规则
- **章节配置**
  - chapters.ts文件定义文档章节结构与URL映射
  - 支持动态加载远程文档内容
  - 提供文档导航与搜索功能

- **排除规则**
  - .gitignore文件包含.qoder/排除规则
  - 防止文档系统生成的临时文件被版本控制
  - 保持仓库整洁与版本历史清晰

```mermaid
flowchart TD
A["用户访问帮助文档"] --> B["chapters.ts解析章节配置"]
B --> C["加载远程Qoder文档内容"]
C --> D["显示Markdown文档"]
D --> E["用户交互与反馈"]
E --> F["文档系统记录访问日志"]
...
- 文档系统依赖
  - 外部Qoder系统提供文档托管与协作功能
  - .gitignore 排除规则确保版本控制整洁
...
QODER["外部Qoder系统"] --> DOC_CONTENT["文档内容"]
CHAPTERS["chapters.ts"] --> QODER
GITIGNORE[".gitignore"] --> QODER
```
...
- 运行时性能
  ...
  - 文档系统采用懒加载机制，提升页面响应速度
- 状态管理
  - Zustand 无样板代码，状态更新粒度可控，减少不必要重渲染
- 文档系统性能
  - 外部Qoder系统提供CDN加速与缓存优化
  - .gitignore排除规则减少版本控制开销
...
**重要更新**：文档管理系统已成功迁移到外部Qoder系统，通过chapters.ts配置实现与设计器的无缝集成，.gitignore排除规则确保版本控制的整洁性。这一变更提升了文档协作效率，简化了文档管理流程，同时保持了技术栈的一致性与可维护性。

## 附录
- 开发与构建建议
  - 使用 Vite 的本地别名指向 SDK 源码，便于联调
  - 在生产构建中开启最小化与 sourcemap，便于问题追踪
  - 定期检查外部Qoder系统的连接状态与文档更新
- 发布与分发
  - 通过 npm 发布 ESM/CJS 双产物，满足不同消费场景
  - 保持 external 依赖明确，避免重复打包
  - 确保文档系统配置与版本发布流程协调一致
- 文档管理最佳实践
  - 利用Qoder系统的协作功能提升团队效率
  - 定期清理.gitignore中排除的临时文件
  - 维护chapters.ts配置与实际文档内容的一致性