...
## 更新摘要
**变更内容**
- 新增字体大小等比缩放机制，提升表格高度计算的准确性
- 更新表头高度和行高计算方法，从固定常量改为基于字体大小的比例缩放
- 改进表格高度测量逻辑，增强浏览器环境下的真实测量精度
- 优化合计行处理方案，支持字体缩放下的动态高度计算
...
## 结论
本方案通过"渲染后测量 + 服务器端估算"的双轨制，结合严谨的列宽计算与防御性分页策略，在保证跨页视觉一致性的同时兼顾了性能与稳定性。新增的字体大小等比缩放机制显著提升了表格高度计算的准确性，使表格在不同字体设置下都能保持精确的布局。合计行与额外行的灵活配置进一步增强了表格在打印场景下的表达力。建议在设计器与生产环境中分别采用合适的测量策略，并配合合理的样式与布局参数，获得最佳的渲染效果。