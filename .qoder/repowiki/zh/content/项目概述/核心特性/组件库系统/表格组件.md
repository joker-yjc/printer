...
## 更新摘要
**变更内容**
- 新增列级样式系统支持，包括 TableColumnStyle 和 TableHeaderStyle 接口
- 新增 column.style 和 headerStyle 属性支持
- 增强表头和数据单元格的样式定制能力
- 更新渲染器样式合并机制，支持列级样式优先级
...
- 设计期（设计器）：提供列配置、样式配置、列宽拖拽、合计额外行配置等交互
...
ColumnSection["TableColumnSection.tsx<br/>列配置与合计额外行"]
...
## 架构概览
表格组件的运行流程如下：
- 设计期：用户通过列配置面板设置列、合计、样式；通过预览查看列宽与表头效果
...
MergeStyles --> RenderHeader["渲染表头<th>"]
...
```mermaid
graph LR
Types["types.ts<br/>新增样式接口"] --> Renderer["TableRenderer.ts"]
...
## 结论
表格组件通过"设计期配置 + 渲染期精确计算 + 跨页智能拆分"的架构，实现了复杂报表场景下的高可用与高一致性。其列宽自动计算、合计与额外行、跨页重复表头等能力，满足了打印场景对布局与数据表达的严格要求。**新增的列级样式系统进一步增强了表格的视觉表现力，为不同业务场景提供了更精细的样式控制能力。**
