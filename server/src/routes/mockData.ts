...
const mockDataStore: MockData[] = [...defaultMockData];

router.post('/', (req, res, next) => {
  try {
    const body = req.body as MockData;
    const id = body.id || uuid();
    const mock: MockData = { ...body, id };
...
router.get('/', (req, res) => {
  const { name, schemaId, templateId } = req.query;
  let result = mockDataStore;

  if (typeof name === 'string' && name.trim()) {
    result = result.filter((m) => m.name.includes(name.trim()));
  }
  if (typeof schemaId === 'string' && schemaId.trim()) {
    result = result.filter((m) => m.schemaId === schemaId.trim());
  }
  if (typeof templateId === 'string' && templateId.trim()) {
    result = result.filter((m) => m.templateId === templateId.trim());
  }
...
router.get('/:id', (req, res) => {
  const mock = mockDataStore.find((m) => m.id === req.params.id);
  if (!mock) {
...
router.put('/:id', (req, res) => {
  const index = mockDataStore.findIndex((m) => m.id === req.params.id);
  if (index === -1) {
    return res.status(404).json({
      code: 'NOT_FOUND',
      message: 'Mock data not found'
  ...
  const updated: MockData = { ...(req.body as MockData), id: req.params.id };
  mockDataStore[index] = updated;
  res.json(updated);
});
